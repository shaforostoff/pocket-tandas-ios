...
//  device fall back to matching by filename within the playlist's folder;
//  anything still unresolved is skipped rather than crashing.
//
...
enum PlaylistParser {
    ...
    /// Resolve a single playlist entry to an existing file URL, or nil.
    static func resolve(_ entry: String, relativeTo directory: URL) -> URL? {
        let fm = FileManager.default

        if entry.hasPrefix("file://"), let url = URL(string: entry), fm.fileExists(atPath: url.path) {
            return url
        }
        ...
        if normalized.hasPrefix("/") {
            let absolute = URL(fileURLWithPath: normalized)
