...
- **Suspension kills the radio (observed):** with nothing playing there is no background audio to keep the receiver alive, so a lock-screen suspension leaves its MCSession *and* advertiser permanently dead — unlocking does not revive them, only new objects do. Mitigations: (1) `PeerLink` watches `didEnterBackground` → `didBecomeActive` and rebuilds the session + advertiser/browser whenever it returns from the background with no connected peer — keyed on a real background transition, so a Control Center pull or the local-network alert can't cut off a handshake; (2) two opt-in launcher toggles, both OFF by default and both released after 30 minutes (window restarts on each return to the foreground) — see [StayAwake.swift](Pocket Tandas/Audio/StayAwake.swift): **Screen Stays Awake** (idle timer disabled, so no auto-lock) and **Silent Keep-Alive Audio** (`SilentKeepAlive` renders looping digital silence on its own AVAudioEngine, so the declared `audio` background mode keeps the app running with the screen off; runs only in the remote modes and only while the PlaybackEngine is idle, since real playback already prevents suspension). The keep-alive carries App Store guideline 2.5.4 risk (background audio that isn't user-facing) — hence off by default and time-limited. Remaining limitation with both toggles off: a locked receiver with nothing playing is unreachable until it is unlocked.
